...
const steps: Record<ModeId, Step[]> = {
  sber: [
    ...
    { screenId: "external",      title: "Внешний резерв",               narration: "Профили внешнего резерва Точки Сборки — студенты, специалисты, эксперты, готовые команды — с оценкой потенциала и готовности." },
  ...
  rosatom: [
    ...
    { screenId: "graph",         title: "Граф инженерных знаний",
      narration: "Эксперт → экспертиза → проект → технология → наставник → следующее поколение инженеров. Знания компании становятся видимыми.",
      employeeNarration: "Эксперт → экспертиза → проект → технология → наставник → следующее поколение инженеров. На этом графе есть место и для вас — платформа показывает, как в него войти." },
  ...
  yandex: [
    ...
    { screenId: "knowledgeGraph", title: "Граф знаний",
      narration: "Не Dashboard — живая карта организации. Исследования, продукты, команды, эксперты, экспертиза, технологии — каждый завершённый проект становится вершиной графа, а не закрытым тикетом.",
