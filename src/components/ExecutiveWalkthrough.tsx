...
const steps: Record<ModeId, Step[]> = {
  sber: [
    { screenId: "problem",       title: "Проблема",                     narration: "Совет директоров снова спросит, почему трансформация буксует. Стратегия есть, специалистов для неё — нет. Это первый вопрос, на который отвечает эта демонстрация." },
    { screenId: "memory",        title: "Организационная память",       narration: "Мама печёт пирог по рецепту у себя в голове — и когда она перестаёт готовить, следующее поколение начинает всё сначала. После каждого завершённого проекта организация либо становится умнее, либо начинает следующий проект заново." },
    ...
    { screenId: "employee",      title: "Анализ готовности",           narration: "GigaChat анализирует профиль сотрудника. Готовность к целевой роли — 62%. Четыре критических пробела видны мгновенно." },
    ...
    { screenId: "plan",           title: "План развития",                narration: "30/60/90 дней — конкретный план роста, а не абстрактное развитие. Каждый шаг привязан к реальному пробелу и реальному следующему проекту." },
    ...
    { screenId: "ecosystem",     title: "Внешняя экосистема",           narration: "Подключаем Точку Сборки. Покрытие критической экспертизы: 67% → 89%. Нажмите кнопку и посмотрите сами." },
    { screenId: "external",      title: "Внешний резерв",               narration: "Дефицит внутри не значит дефицит вообще. Прежде чем открывать вакансию на три месяца — карта тех, кто готов войти в проект на этой неделе." },
    { screenId: "orchestration", title: "Agentic AI",                   narration: "Раньше ответ на кадровый вопрос собирали неделями — из HR, финансов, проектного офиса. Семь агентов делают то же самое за минуты. Решение всё равно принимает человек." },
  ...
  vk: [
    { screenId: "problem",    title: "Проблема",                narration: "Почему две одинаково талантливые команды делают продукты с разницей в полгода? Обычно ответ прячется в том, что одна искала специалиста месяц, а другая нашла его за день." },
    { screenId: "memory",     title: "Организационная память",  narration: "После каждого завершённого проекта организация либо становится умнее — либо начинает следующий проект заново. Разница в полгода между командами объясняется именно этим." },
    ...
    { screenId: "external",   title: "Внешний резерв",         narration: "Дефицит внутри не значит дефицит вообще. Карта тех, кто готов войти в проект на этой неделе, а не через три месяца найма." },
    { screenId: "pipeline",   title: "Конвейер талантов",      narration: "Найм закрывает вакансию. Конвейер закрывает разрыв на годы вперёд — кого готовить наставником уже сейчас, чтобы не искать его на рынке через год." },
  ...
  rosatom: [
    { screenId: "problem",       title: "Проблема",
      narration: "Через пять лет уйдут люди, которые знают то, чего никто больше не знает. Пять стратегических программ Росатома уже сегодня нуждаются в специалистах, которых не хватает.",
      employeeNarration: "Через пять лет уйдут люди, которые знают то, чего никто больше не знает. Возможно, один из них мог бы стать вашим наставником — если узнать об этом вовремя." },
    { screenId: "memory",        title: "Организационная память",
      narration: "Инженерная школа всегда держалась не на архиве чертежей, а на людях, которые передавали мышление ученикам. После каждого проекта организация либо становится умнее, либо начинает следующий заново.",
      employeeNarration: "То же самое верно и для вас: рядом уже есть люди, у которых можно научиться тому, что нельзя прочитать ни в одной инструкции." },
    ...
    { screenId: "map",           title: "Карта критической экспертизы",
      narration: "Тепловая карта критической экспертизы страны — от реакторных технологий до AI для инженерии. Где риск дефицита — сразу видно.",
    ...
    { screenId: "graph",         title: "Граф инженерных знаний",
      narration: "Найдите узел «О. Денисов» — единственный человек в организации, знающий литиевую металлургию так, как нужно программе. Если он уйдёт раньше, чем появится преемник, эта ветка графа обрывается навсегда.",
      employeeNarration: "О. Денисов — единственный носитель этой экспертизы. Учиться у него сейчас значит занять место в графе, которое иначе просто исчезнет." },
    ...
    { screenId: "forecast",      title: "Прогноз экспертизы 2035",
  ...
  yandex: [
    { screenId: "problem",        title: "Проблема",
      narration: "Почему одна команда каждый раз начинает исследования заново — как будто до неё никто этого не делал? Похожие эксперименты уже идут в трёх командах, и никто об этом не знает.",
    ...
    { screenId: "memory",         title: "Организационная память",
      narration: "После каждого завершённого проекта организация либо становится умнее — либо начинает следующий проект заново. Три команды, повторяющие одно исследование, — это организация, которая начинает заново." },
    { screenId: "why",            title: "Два взгляда",
    ...
    { screenId: "knowledgeGraph", title: "Граф знаний",
      narration: "«Исследование: генеративный поиск» и «Маркет: генеративный поиск» выглядят как одна линия. Но рядом, невидимо для этого графа, ещё две команды ведут те же эксперименты.",
      employeeNarration: "Если вы начинаете что-то похожее на этот узел — нажмите и посмотрите, кто уже прошёл этот путь. Возможно, неделя работы уже сделана за вас." },
  ...
  future2035: [
    { screenId: "problem",      title: "Проблема",
      narration: "Как понять, где именно организация теряет скорость реализации стратегии — до того, как это станет кризисом на совете директоров? Это не вопрос Сбера, Росатома, VK или Яндекса. Это вопрос любой крупной организации.",
    ...
    { screenId: "memory",       title: "Организационная память",
      narration: "После каждого завершённого проекта любая организация либо становится умнее, либо начинает следующий проект заново. Это универсальный принцип — не особенность одной отрасли." },
    { screenId: "why",          title: "Два взгляда",
