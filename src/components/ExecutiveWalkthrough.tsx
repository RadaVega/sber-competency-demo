...
const steps: Record<ModeId, Step[]> = {
  sber: [
    ...
    { screenId: "convergence",   title: "Две реальности",               narration: "Компания видит: стратегия не реализуется. Сотрудник видит: не понимаю, зачем мне всё это. Они работают в одной компании — но живут в разных реальностях. Платформа не управляет людьми: она синхронизирует цели организации и стремления человека в едином цикле." },
  ...
  rosatom: [
    ...
    { screenId: "dashboard",     title: "Стратегическая реализация", narration: "Это не HR-дашборд. Пять стратегических программ, их прогресс, риски и дефицит специалистов — на одном экране для директора программы." },
    { screenId: "map",           title: "Карта компетенций",        narration: "Тепловая карта критических компетенций страны — от реакторных технологий до AI для инженерии. Где риск дефицита — сразу видно." },
    { screenId: "knowledgeRisk", title: "Риск потери знаний",       narration: "Уникальных экспертов мало, средний возраст растёт. Платформа считает вероятность потери критической экспертизы и говорит, что делать." },
    { screenId: "graph",         title: "Граф инженерных знаний",   narration: "Эксперт → компетенция → проект → технология → наставник → следующее поколение инженеров. Знания компании становятся видимыми." },
    ...
    { screenId: "teamBuilder",   title: "Формирование команды",     narration: "Выбираем программу — AI собирает команду: кто уже внутри, кого не хватает, какой риск у проекта." },
    { screenId: "aiCenter",      title: "Центр AI-управления",      narration: "Какие программы под риском? Каких специалистов не хватает? Директор задаёт вопрос — получает ответ с данными и графиками." },
    { screenId: "forecast",      title: "Прогноз компетенций 2035", narration: "Сегодня, через 3, 5 и 10 лет — сколько специалистов есть и сколько понадобится. Подготовку нужно начинать заранее, не в момент кризиса." },
