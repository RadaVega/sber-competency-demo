...
export function SberApp({ active, onChangeScreen }: { active: SberScreenId; onChangeScreen: (id: SberScreenId) => void }) {
  ...
  return (
    <>
      {active === "ecosystem-concept" && <StrategicEcosystemScreen mode={mode} onBack={() => {}} onNext={() => go("dashboard")} />}
      ...
      {active === "pipeline"          && <TalentPipelineScreen mode={mode} onBack={() => go("external")} onNext={() => go("orchestration")} />}
      {active === "orchestration"     && <OrchestrationScreen onBack={() => go("pipeline")} onNext={() => go("executive")} accentColor={mode.accentColor} />}
...
export function useSberScreenState() {
  return useState<SberScreenId>("ecosystem-concept");
}
